  ...
  class AstarDb
    ...
    def self.h0
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=0 )"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.h1
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=1 )"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.h2
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=2 )"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.h3
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=3 )"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.h4
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=4 )"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.h5
      sql = "Select * from pgr_astar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            ARRAY[1,2,3,4,5], ARRAY[6,7,8,9,10], heuristic :=5 )"
      ActiveRecord::Base.connection.execute(sql)
    end
    ...
    def self.pgrbdAstar
      sql = "Select * from pgr_bdAstar('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            1, 1011);"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.pgrDjstra
      sql = "Select * from pgr_dijkstra('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            1, 1011);"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.pgrbdDjstra
      sql = "Select * from pgr_bdDijkstra('SELECT gid as id, source, target, cost, reverse_cost, x1, y1, x2, y2 FROM ways',
            1, 1011);"
      ActiveRecord::Base.connection.execute(sql)
    end

    def self.PriorityQueueCase1
      def initialize
        @elements = []
      end
